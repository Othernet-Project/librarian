...
def with_mock_open(fn):
    ...
    def wrapper(*args, **kwargs):
        m = mock.mock_open()
        with mock.patch('librarian.core.metadata.open', m, create=True):
...
def test_add_missing_keys_has_return():
    ...
    d = {}
    ret = metadata.add_missing_keys(d)
    assert ret == None


@mock.patch('librarian.core.metadata.REQUIRED_KEYS')
@mock.patch('librarian.core.metadata.json', autospec=True)
def test_convert_returns__added_keys(json, *ignored):
    """ Conversion to json calls ``add_missing_keys()`` on converted value """
    json.loads.return_value = {}
    out = metadata.convert_json('')
    assert out == {
        'url': None,
        'domain': None,
        'title': None,
...
@mock.patch('librarian.core.metadata.os', autospec=True)
@mock.patch('librarian.core.metadata.add_missing_keys', autospec=True)
@mock.patch('librarian.core.metadata.json', autospec=True)
...
def test_meta_attribute_access(*ignored):
    """ Attribute access to metadata keys should be possible """
    meta = metadata.Meta({'foo': 'bar', 'baz': 1}, 'foo')
    assert meta['foo'] == 'bar'
    assert meta.baz == 1
...
def test_meta_set_key_updates_original(*ignored):
    """ The original metadata dict is updated when Meta object is updated """
    data = {}
    meta = metadata.Meta(data, 'foo')
    meta['missing'] = 'not anymore'
...
def test_meta_get_key(*ignored):
    ...
    meta = metadata.Meta({'foo': 'bar'}, 'foo')
...
def test_meta_cache_cover_file_path(mock_open, os, *ignored):
    ...
    os.path.normpath.side_effect = noop
    meta = metadata.Meta({'md5': 'md5'}, 'covers_dir')
...
@mock.patch('librarian.core.metadata.json', autospec=True)
@mock.patch('librarian.core.metadata.zipfile', autospec=True)
@with_mock_open
def test_extact_image_looks_for_frist_image(mock_open, zipfile, *ignored):
    ...
    meta = metadata.Meta({'md5': 'md5'}, 'covers_dir', zip_path='foo.zip')
...
@mock.patch('librarian.core.metadata.os', autospec=True)
@mock.patch('librarian.core.metadata.glob', autospec=True)
...
def test_lang_property(*ignored):
    ...
    meta = metadata.Meta({'language': 'foo'}, 'covers_dir')
...
def test_human_license_property(*ignored):
    ...
    meta = metadata.Meta({}, 'covers_dir')
    with attr_overrides(metadata, LICENSES=faux_licenses):
...
@mock.patch('librarian.core.metadata.glob', autospec=True)
@mock.patch('librarian.core.metadata.os', autospec=True)
@mock.patch.object(metadata.Meta, 'get_cover_path')
@mock.patch.object(metadata.Meta, 'extract_image')
@mock.patch.object(metadata.Meta, 'cache_cover')
def test_image_property(cache_cover, extract_image, get_cover_path, *ignored):
    ...
    meta = metadata.Meta({}, 'covers_dir', zip_path='foo.zip')
    res = meta.image
